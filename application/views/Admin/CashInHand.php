	...
	<body class="vertical-layout vertical-menu-modern 2-columns   fixed-navbar" data-open="click" data-menu="vertical-menu-modern" data-col="2-columns">
		...
		<div class="app-content content">
			...
			<div class="content-wrapper">
				...
				<div class="row mb-2">
					<div class="col-12">
						<h5><a href="<?= base_url($this->data->controller); ?>/Dashboard"><?= $this->data->title; ?></a> / BAD CASH IN HAND</h5>
					
                        
                        <br/>

                        <!--Form start here -->
                        <form action="<?php echo base_url('Admin/CashInHand'); ?>" method="post">
								...
								<div class="card" >
									...
									<div class="card-content collapse show " style="margin-top:-40px;">
										<div class="card-body" id="printableArea">
											<h4 class="card-title" id="basic-layout-form-center">CASH IN HAND
											...
											<table style="width:100%">
												...
												<tbody>
													<tr <?php if(isset($fyyear)) { } else {?> style="display:none; " <?php } ?> >
														<td></td>
														<td colspan="3">
															<span>TO OPENING BAL.B/F <?= date("d/m/Y",strtotime($start_date)) ?>
																<!--<?=$total_debit_amount; ?> = <?=$total_credit_amount; ?> = <?= $total_debit_amount-$total_credit_amount; ?>-->
															</span>	
														</td>
														...
														<td>
															...
															{
																$query = $this->db->where(['entrycode'=>'BALSOC','financial_year'=>"2022-2023"])->get('tbl_balance');
																$data['list'] = $query->result();
																$Opening_Bal = $data['list'][0]->amount;
															}
															else if($fyyear=="2024-2025")
															{
																$query = $this->db->where(['entrycode'=>'BALSOC','financial_year'=>"2023-2024"])->get('tbl_balance');
																$data['list'] = $query->result();
																$Opening_Bal = $data['list'][0]->amount;
															}
															// echo $Opening_Bal;
															echo sprintf('%0.2f', abs($Opening_Bal));
															?>
														</td>
														<td></td>
													</tr>
													...
													<?php
														...
														if(!empty($list))
														{
															foreach ($list as $item)
															{
																...
																// Start - check if this entry is in FY Year 
																...
																// hide data after end date
																
																if($date3>$date2) {
																	continue;
																}
																
																// End - check if this entry is in FY Year 
																...
																else
																{
																	...
																	$total = $total_Credit_value-$total_Debit_value;
																	
																	//hide total before start date
																	if($date3<$date1) {
																		$total_Debit_value =0; 
																		$total_Credit_value =0; 
																	}
															...
															?>
															<tr <?php if($displaymode!="") { ?> style="display:<?= $displaymode ?>; " <?php } ?> > 
																...
																<td>
																	...
																	<?php
																		if($total_Debit_value>$total_Credit_value)
																		{
													...
													?>
													<tr  <?php if(isset($fyyear)) { echo ""; } else { echo 'style="display:none;"'; } ?>  style="border-top:dotted;border-bottom:dotted;">
														<td></td>
														
														<td colspan="4">
														
															<b><span>BALANCE (As on <?= date("d/m/Y",strtotime($end_date)) ?>)</span>	</b>
														...
														<td>
															<b><?php
																// $creditSum = (float)$Opening_Bal+$total_Credit_value; 
																$creditSum = (float)$Opening_Bal+$total_Credit_value; 
														...
														<td>
															<b><?php 
																...
																{ 
																	...
																	$query = $this->db->where(['entrycode'=>'BALSOC','financial_year'=>$financial_year])->get('tbl_balance');
																	if ($query->num_rows())
																	{
																		$data['list'] = $query->result();
