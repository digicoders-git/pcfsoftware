							...
							<div class="col-md-12">
								<div class="card" id="printableArea">
									<div class="card-header">
										<h4 class="card-title" id="basic-layout-form-center">Subsidiary Loans Ledger <span> (As on <?php echo date('31/03/2023');?>)</span></h4>
										<br>
										
										<!-- Filter Form Start Here-->
										<form method="post" action="<?=base_url('Admin/SubsidiaryLoanLedger')?>">
												...
												<tbody>
													<?php
														...
														foreach ($list as $item)
														{
															...
															if(!empty($item->total_debit))
															{
																$total_debit_loan+=$item->total_debit;
															}
															if(!empty($item->total_credit))
															{
																$total_credit_loan+=$item->total_credit;
															}
														...
														<tr>
															...
															<td>
																<?php 
																	if(!empty($item->total_debit))
																	{
																		echo number_format($item->total_debit, 2, '.', '');
																	}
																	else 
																	{
																		echo "0.00";
													...
													<tr style="border-top:dotted;border-bottom:dotted;">
														...
														<td colspan='2'>
															<b><?php
																$debitSum = $total_debit_loan; 
																echo number_format($debitSum, 2, '.', '')." Dr";
															?></b>
														</td>
														<td>
															<b><?php
																$creditSum = $total_credit_loan; 
